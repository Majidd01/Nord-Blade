...
if (!category) {
  console.error("Usage: node import-product-images.mjs <category>");
  process.exit(1);
}
...
const manifest = JSON.parse(
  await readFile(new URL(`./${category.replace(/-knives$/, "-knife")}-manifest.json`, import.meta.url), "utf8"),
);

const outDir = path.join(process.cwd(), "public", "images", "products", category);
...
for (let i = 0; i < manifest.length; i++) {
  ...
  for (const [imgIndex, url] of [item.image, item.image2].filter(Boolean).entries()) {
    ...
    const filename = `${prefix}-${index}${suffix}.jpg`;
    ...
    try {
      ...
      const localPath = `/images/products/${category}/${filename}`;
